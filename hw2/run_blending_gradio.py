...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    foreground_laplacian = F.conv2d(foreground_img, laplacian_kernel, padding=1, groups=foreground_img.shape[1])
    blended_laplacian = F.conv2d(blended_img, laplacian_kernel, padding=1, groups=blended_img.shape[1])

    foreground_mask = foreground_mask.expand_as(foreground_laplacian).bool()
    background_mask = background_mask.expand_as(blended_laplacian).bool()

    foreground_values = foreground_laplacian[foreground_mask]
    blended_values = blended_laplacian[background_mask]

    if foreground_values.numel() == 0 or blended_values.numel() == 0:
        return loss

    compare_len = min(foreground_values.numel(), blended_values.numel())
    loss = F.mse_loss(foreground_values[:compare_len], blended_values[:compare_len])
