...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        if knn_points is not None:
            ...
            dmat = torch.cdist(points3D_xyz, points3D_xyz, p=2)
            dists = torch.topk(dmat, K, largest=False, dim=1).values.pow(2).unsqueeze(0)
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        scales = torch.exp(self.scales).clamp(min=1e-4, max=1.0)
