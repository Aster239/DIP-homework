...
def parse_args():
    ...
    parser.add_argument('--max_points', type=int, default=None,
                      help='Maximum number of COLMAP points to keep')
    parser.add_argument('--downsample_factor', type=int, default=8,
                      help='Image and intrinsic downsample factor')
...
def main():
    ...
    dataset = ColmapDataset(
        ...
        maximum_pts_num=args.max_points
    )
    print(f"COLMAP points used: {len(dataset.points3D_xyz)}")
    ...
    trainer.save_checkpoint(start_epoch + config.num_epochs)
    if device.type == 'cuda':
        print(f"CUDA max memory allocated MiB: {torch.cuda.max_memory_allocated(device) / 1024**2:.2f}")
        print(f"CUDA max memory reserved MiB: {torch.cuda.max_memory_reserved(device) / 1024**2:.2f}")
